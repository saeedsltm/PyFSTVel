...
class Main():
    ...
    def run_fst_pso(self):
        ...
        if fst_pso_par["MP_FLAG"] != 0:
            FP.set_parallel_fitness(self.parallel_fitness_function)
            if self.num_it != 0:
                result = FP.solve_with_fstpso(max_iter=self.num_it, max_iter_without_new_global_best=10 ,dump_best_fitness=best_fitness_out)
            else:
                result = FP.solve_with_fstpso(max_iter_without_new_global_best=10, dump_best_fitness=best_fitness_out)
        else:
